...
- runs declared `workflows.<name>.proof.seam_observations[]` after readiness and before teardown;
  ...
  attestation; only a verified attestation becomes `exercised` dependency evidence
...
- when a sole fixed `runtime_listener` conflict is established, text output reports `Host port already in use`, names the requested and active execution modes plus exact endpoint, and suggests a `--host-port <free port>` rerun only when the selected lane's normal option preflight admits host-port overrides; native and container requests use the same diagnosis, while mixed ownership conflicts keep the broader `Active execution conflict` title and add port-specific remediation
...
- when the selected or default workflow task closure declares explicit
  `tasks.<name>.requirements.checks`, `ota check` also runs those prerequisite checks without
  broadening into runtime, tool, or env diagnosis
