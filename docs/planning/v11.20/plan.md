...
Status: planned. This follows V11.19. It is a narrow policy refinement of the shipped optional
...
- every reachable declared replay input is task-qualified and its observed identity is computed
...
- for a task invocation, every task rule whose subject is reachable in the selected task dependency
  closure applies;
- for a workflow invocation, the exact workflow rule applies to the complete selected workflow
  ...
  applies;
- each task rule evaluates only its subject task's own dependency closure, never unrelated inputs
  elsewhere in the parent task or workflow;
...
policy identities, coverage, and typed insufficiency reasons. Doctor is the first diagnostic
carrier; dry-run, `ota run`, and `ota up` consume the same decision before dependency hydration or
task startup. A `review` decision refuses every execution surface in V11.20, including attended
native human execution, agent execution, and CI. V11.20 introduces neither a bypass flag nor a
grant-authority mechanism; a future authorization slice may define one explicitly rather than
treating a recorded crossing reason as approval.

CI projection carries only the canonical policy requirement, selected closure, and policy identity.
It never embeds a developer checkout's observed file digest or decision. The generated provider
lane re-runs the same evaluator after checkout, where the declared inputs actually exist.
...
2. Implement one closure-aware replay-input identity-policy evaluator by reusing the existing
   `expected_identity` preflight result; Doctor, dry-run, run, up, and CI projection must not
...
4. Add regressions for matching pins, missing pins, missing artifacts, mismatched artifacts,
   parent-task dependency rule enforcement, per-rule closure isolation, workflow/task rule
   aggregation and precedence, unknown-selector policy findings and governed refusal, unselected
   compatibility, CI checkout re-evaluation, and no execution before refusal.
...
- invoking a parent task or workflow cannot bypass a replay-input identity rule on a reachable
  dependency, and an unrelated pinned input cannot satisfy that dependency's rule;
- missing pins, unreadable inputs, and mismatches are typed, inspectable, and refuse before any
  dependency hydration or task process begins; declared-pin failures refuse regardless of policy
  action;
...
- Doctor, dry-run, run, up, receipt, and CI projection consume one canonical requirement, while
  every provider checkout evaluates its own observed input identities;
