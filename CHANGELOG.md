...
- replaced runtime/tool OS scoping via `required` plus `platforms.<os>.required` with a cleaner `only_on` contract surface, keeping `platforms` for per-OS value overrides only.
