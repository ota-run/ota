...
- improved workspace command observability for JSON usage: `ota workspace doctor`,
  `ota workspace check`, `ota workspace run`, `ota workspace up`, and
  `ota workspace refresh` now accept `--progress-json` to emit live machine-readable stderr
  progress while preserving the final JSON report on stdout, and workspace-run progress now
  surfaces both `task` and resolved `repo_task` when a workspace task binding rewrites the local
  task name
