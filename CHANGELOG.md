...
- expanded HTTP service `runtime.readiness` so repos can now declare probe `method`, request `headers`, accepted `success.status` codes, a required `body.contains` substring, and activation timing controls (`interval`, `timeout`, `retries`, `start_period`); validation, runtime probing, topology JSON output, and canonical docs/examples now follow the same richer readiness contract
...
- added a concrete Studio Phase 1 build plan under `docs/spec/studio/phase1-build-plan.md` so implementation can start from an explicit serve-first execution checklist with deliverables, current file touch points, backend/frontend split, acceptance criteria, and validation rules instead of re-deriving Phase 1 scope from the broader product and architecture specs
- expanded the Studio spec set with concrete build-facing implementation guides for the remaining under-specified areas: `event-schema.md`, `registry-schema.md`, `http-api.md`, and `design-system.md` now define the canonical live-event envelope, repo registry file contract, localhost Studio server endpoints, and the premium full-viewport visual system so another AI can build against explicit Studio truth instead of guessing
- expanded the Studio spec further so build-phase truth now includes an explicit `Studio Home / Repos` surface, Action Center language, a repo-level operation history model, stale-review recovery UX, stricter selected-task trigger boundaries, an explicit evidence-panel shape, and multiple topology inspection modes instead of a graph-only assumption
- tightened the Studio spec freeze again by fixing four implementation-shaping gaps: the canonical layer order is now `Inspect -> Review -> Apply -> Trigger -> Observe`, Studio now has explicit non-default contract-path support, the architecture shell now names the same durable panes as the product spec, and reviewed Studio writes now require a stale-review safety boundary through repo-scoped serialization or equivalent revision checks
- expanded the Studio spec again so the durable app shape now names five panes (`Overview`, `Contract`, `Draft`, `Topology`, `Run / Evidence`), requires topology clickback into contract/inference/evidence truth, and makes the architecture path explicit: existing Ota JSON first, then dedicated Contract View / Draft / Topology APIs once Studio needs harden
- tightened the Studio spec set so the repo-level canon now says explicitly that CLI, Studio, and agents are equal first-party Ota clients, locks the Inspect/Review/Apply/Trigger/Observe product model into the spec index, and calls out cross-shell/cross-agent live activity visibility as part of the later event-driven Studio direction
- added a canonical Studio spec set under `docs/spec/studio/` covering product direction, architecture, and phased roadmap so the next Studio phase can pivot cleanly from the current snapshot MVP to the interactive premium app surface; the spec now explicitly captures the 5-layer Studio model (inspect/review/trigger/observe/apply), equal first-party CLI/Studio/agent positioning, the on-demand session-service event architecture, and the future live agent/terminal activity surface
...
- documented the Studio snapshot payload explicitly in the JSON output reference, so `ota studio` now has a clear embedded data contract (`studio`, `contract_text`, `doctor`, `detect`, `topology`) instead of leaving the HTML snapshot shape implicit
...
- promoted `ota studio --open` into the repo onboarding path so quickstart, README, and example-selection docs now teach the visual contract/topology review surface as part of first-run review instead of leaving Studio buried in reference-only docs
