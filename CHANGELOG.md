...
- added the first narrow backend-provider remote activation slice: when caller and producer share one declared remote backend binding, `address_view: host` now supports `ensure_started`, `restart_ready`, `ensure_running`, and `ensure_ready` for backend-provider producer services only if the matching `backend_provider` extension declares `activation.provider_managed_cleanup: true`; ota now sends provider command contexts (`run`, `activation`, `activation_cleanup`), waits honestly on the fixed host endpoint, and keeps `address_view: topology` and `address_view: internal` out of scope for backend-provider activation
