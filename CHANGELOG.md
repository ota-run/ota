...
- added shell-completion guidance with contract-aware dynamic suggestions so `ota <TAB>` completes commands, `ota run <TAB>` completes only tasks that have one satisfiable shared invocation across the selected repo/member target set, `ota run <task> <TAB>` completes shared task input flags plus constrained values that remain valid across that target set, `--member <TAB>` completes monorepo member names, and workspace task/repo filters complete from the active workspace contract while keeping `ota workspace run <TAB>` and `ota workspace run <task> <TAB>` limited to tasks and shared inputs that stay satisfiable across the available repos.
